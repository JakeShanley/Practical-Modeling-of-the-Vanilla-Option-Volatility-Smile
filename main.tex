...
\newline
CHANGE
The Gauss-Newton method presumes that the objective function is locally quadratic (approximately) in the parameters near the optimal solution\textsuperscript{\cite{Bjorck}}.  From \cite{Gavin}, the step vector at each step for for the Gauss-Newton method can be found by solving 
\begin{equation}
...
Weighting also had significant impact on the trading simulation's profit or loss.  For the Smile function choice, profit and loss details when different weighting schemes are applied are shown in tables \ref{tab:TSLA_Mar_SmileWeighting}, \ref{tab:TSLA_Jan_SmileWeighting}, and \ref{tab:NVDA_Jan_SmileWeighting}.
...
Tables \ref{tab:TSLA_Mar_TwistWeighting}, \ref{tab:TSLA_Jan_TwistWeighting}, and \ref{tab:NVDA_Jan_TwistWeighting} show the affect of weighting choice on profit and loss when modelling with the Twist function.
...
The resulting theoretical values between using the Gauss-Newton and Levenberg-Marquardt algorithms was negligible ($O(10^{-14})$) and thus no end profit and loss difference was observed between the two optimization algorithms under any combination of function, weighting, trading strategy, or dataset.  The question of which algorithm to use, then, just becomes one of efficiency.
\newpar
